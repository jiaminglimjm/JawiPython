اينڤوت = '''gfeabcd adecfb gcedb cef efgdc decabg cbfg edfga fdegcb cf | fc faedg fdage fec
...
ڤڠيرا = {ي:0 اونتوق ي دالم [2, 4, 3, 7]}

اونتوق ݢاريس دالم اينڤوت.split('\n'):
    اونتوق ڤرکاتاءن دالم ݢاريس.split(' | ')[1].split():
        جک len(ڤرکاتاءن) دالم ڤڠيرا:
            ڤڠيرا[len(ڤرکاتاءن)] += 1
...
فوڠسي سوسون_ر (رينتيتن):
    کمبالي str.join('' ,سوسون(رينتيتن))

جاواڤن = 0
اونتوق ݢاريس دالم اينڤوت.split('\n'):
    تکا_تکي ,امڤت_ديݢيت = ݢاريس.split(' | ')
    تکا_تکي = [سوسون_ر(شي) اونتوق شي دالم تکا_تکي.split()]

    ديݢيت = {}
    ساتو = [شي اونتوق شي دالم تکا_تکي جک len(شي) == 2][0]; ديݢيت[(ساتو)]  = 1
    امڤت = [شي اونتوق شي دالم تکا_تکي جک len(شي) == 4][0]; ديݢيت[(امڤت)]  = 4
    توجوه =[شي اونتوق شي دالم تکا_تکي جک len(شي) == 3][0]; ديݢيت[(توجوه)] = 7
    لاڤن =  [شي اونتوق شي دالم تکا_تکي جک len(شي) == 7][0]; ديݢيت[(لاڤن)]   = 8

    ليم = [شي اونتوق شي دالم تکا_تکي جک len(شي) == 5] # 2 ,3 ,5
    انم = [شي اونتوق شي دالم تکا_تکي جک len(شي) == 6] # 0 ,6 ,9

    اونتوق د دالم ليم:
        جک سموا((ک دالم د) اونتوق ک دالم ساتو):
            ديݢيت[(د)] = 3

    اونتوق د دالم انم:
        جک سموا((ک دالم د) اونتوق ک دالم امڤت):
            ديݢيت[(د)] = 9


    اونتوق د دالم انم:
        جک بوکن د دالم ديݢيت دان سموا((ک دالم د) اونتوق ک دالم ساتو):
            ديݢيت[(د)] = 0

    اونتوق د دالم انم:
        جک بوکن د دالم ديݢيت:
            ديݢيت[(د)] = 6
            انم6 = د

    اونتوق د دالم ليم:
        جک بوکن د دالم ديݢيت دان len([0 اونتوق ک دالم انم6 جک (ک دالم د)]) == 5:
            ديݢيت[(د)] = 5
    ...
    نومبور = 0
    اونتوق ي ,د دالم مڠڠکاکن(امڤت_ديݢيت.split()):
        نومبور += ديݢيت[سوسون_ر(د)] * 10 ** ( 3 - ي )
    جاواڤن += نومبور
